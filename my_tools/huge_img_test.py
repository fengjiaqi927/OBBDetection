...
def main():
    ...
    for img_name in img_list:
        ...
        for bbox, label, score in zip(bboxes, labels, scores):
            img = cv2.drawContours(img, [np.int64(np.array_split(bbox, 4, axis=0))], -1, (255, 0, 255), thickness=5)
            text = f'{model.CLASSES[label]}|{score:.2f}'
            img = cv2.putText(img, text, (int(min(bbox[0::2])), int(min(bbox[1::2]))), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 255, 0), thickness=2)
